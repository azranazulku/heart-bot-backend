...
if cfg.feats:
    ...
    preds_dict = output["preds"]
    conditions = list(preds_dict.keys())
    probs = list(preds_dict.values())

    plt.figure(figsize=(12, 6))
    sns.barplot(x=conditions, y=probs)
    plt.ylabel("Predicted Probability")
    plt.title("Predicted Pathologies from Chest X-ray")
